    ...
    def __init__(self, review_fname: str, product_fname:str, img_loaded:bool):
        ...
        for i in tqdm(range(len(lines))):
            ...
            for ft in ['brand', 'feature']:
                if ft in dp.keys():
                    meta += " ".join(dp[ft])
            self._product[asin] = {'product':product,'description':description,'meta':meta}
        f_product.close()
        ...
        for i in tqdm(range(len(lines))):
            ...
            if asin in self._product.keys():     
                if 'reviewText' in dp.keys():
                    review = dp['reviewText'] + dp['summary'] if 'summary' in dp.keys() else dp['reviewText']
                    score = float(dp['overall'])
                    verified = dp['verified']
                    reviewerID = dp['reviewerID']
                    reviewername = dp['reviewerName'] if 'revieweName' in dp.keys() else 'Anonymous'
                else: 
                    continue

                self._data[reviewerID] = {'reviewerName':reviewername, 'verified':verified, \
                    'asin':asin, 'score':score, 'review':review}
                self._data[reviewerID] = merge_dict(self._data[reviewerID], self._product[asin])
...
if __name__ =='__main__':
    Fashion = AmazonDataset(review_fname="Dataset/Amazon Review/AMAZON_FASHION.json.gz",
